...
def get_imlist(data_root, test_dataset='roxford5k'):
    dataset_root = os.path.join(data_root, 'datasets')
    cfg = configdataset(test_dataset, dataset_root)

    img_root = os.path.join(dataset_root, test_dataset, 'jpg')
    imlist = []
    for im in cfg['imlist']:
        im_name = im + '.jpg'
        im_path = os.path.join(img_root, im_name)
        imlist.append(im_path)

    qimlist = []
    for qim in cfg['qimlist']:
        qim_name = qim + '.jpg'
        qim_path = os.path.join(img_root, qim_name)
        qimlist.append(qim_path)

    return imlist, qimlist


def run_eva(feature_path, data_root, test_dataset='roxford5k', b_download_datasets=False):
    # ---------------------------------------------------------------------
